...
用法：
  ...
  # 完整训练
  python train.py --epochs 100 --batch-size 256 --amp
    ...
    def __getitem__(self, idx: int):
        ...
        if ligand_path.exists():
            ...
            ligand_data = PyGData(
                ...
                num_nodes=1,  # PyG collate 需要所有 Data 有一致的 key 集
            )
        ...
        return {
            ...
            # Km 目标（[0,1] 归一化在 ThreeHeadLoss 内部完成）
...
class Trainer:
    ...
    def _model_forward(self, batch_gpu: dict) -> dict:
        """模型前向（三头模式，传入温度给 EyringKcatHead）"""
        return self.model(
            ligand_data=batch_gpu["ligand_data"],
            seq_embed=batch_gpu["seq_embed"],
...
def main():
    ...
    parser.add_argument("--kcat-ode-steps", type=int, default=10,
                        help="ODE steps for kcat head (default=10)")
    ...
    if use_three_head:
        log.info("Using THREE-HEAD model: BindingDualHead + KcatHead(BINN+ODE+Eyring) + KmHead")
        model = Trenzition(
        ...
        log.info(f"Three-Head: hidden={args.hidden_dim}, kcat_ODE_steps={args.kcat_ode_steps}")
    else:
