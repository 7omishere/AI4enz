...
def evaluate_trenzition(model, loader, device):
    ...
    pkd_preds = np.array(all_preds_pkd) * 12.0
    pkd_targets = np.array(all_targets_pkd) * 12.0
    kcat_preds = np.array(all_preds_kcat) * 15.0 - 7.0
    kcat_targets = np.array(all_targets_kcat) * 15.0 - 7.0

    return {
        "pkd": {"preds": pkd_preds, "targets": pkd_targets},
        "kcat": {"preds": kcat_preds, "targets": kcat_targets},
    }
...
def main():
    ...
    # ── 2. Trenzition ──
    log.info("Evaluating Trenzition...")
    ...
    for task in ["pkd", "kcat"]:
        ...
        for name, m in baseline_results.get(task, {}).items():
            all_models[name] = m
